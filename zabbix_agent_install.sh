...
chown root:$ZABBIX_USER_NAME /etc/zabbix/zabbix_agentd.d/

# Log dir
mkdir -p $ZABBIX_LOG_DIR_LOCATION
chown root:$ZABBIX_USER_NAME $ZABBIX_LOG_DIR_LOCATION

# Backup old zabbix configuration
mv $ZABBIX_CONF_LOCATION "$ZABBIX_CONF_LOCATION".old
touch $ZABBIX_CONF_LOCATION
chown root:$ZABBIX_USER_NAME $ZABBIX_CONF_LOCATION
...
if [ "$ZABBIX_PKS_ENABLED" -ne 0 ]; then
	...
	" 
	echo $ZABBIX_PKS_CONFIGUARION >> $ZABBIX_CONF_LOCATION
fi
...
if command -v ufw >/dev/null; then
	echo "Allow zabbix through firewall. UFW"
	ufw allow from $ZABBIX_SERVER/32 to any port $ZABBIX_PORT comment "Allow zabbix-agent"
